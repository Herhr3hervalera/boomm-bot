...
bot = telebot.TeleBot('2053050673:AAGfFUMCY72XTUZ1qrlw4JN_i4E3PyxwhU4')
...
def authoring(message):
	bot.send_message(message.chat.id, "Автор бота: @SMOKE_SOFTWARE. По поводу сотрудничества и рекламы писать менеджеру.")

@bot.message_handler(commands=['id'])
def id(message):
	bot.send_message(message.chat.id, '[@smoke_software]Ваш ID: ' + str(message.chat.id))
...
bot.polling(True) #слито в @smoke_software
